...
def build_pgo(ref_id, opt_pose, poses_all, pos_cov, device='cuda:0'):
    ...
    # ##### 3cov #######
    # pos_cov = pos_cov.reshape(-1, n, n-1, 3) * 10.0 # [bs, n, n-1, 3]
    # cov_inv = (1.0 / pos_cov) # [bs, n, n-1, 3]
...
def run_pgo(mn_out_pos, mn_out_cov, prior_pose, robot_num=16, device='cuda:0'):
    '''
    input:
        mn_out_pos: torch.tensor, [batch_size*robot_num, (robot_num-1), 3]
        mn_out_cov: torch.tensor, [batch_size*robot_num, (robot_num-1), 1]
        prior_pose: torch.tensor, [batch_size*robot_num*(robot_num-1), 7]
        robot_num: int
        device: str, device id
    output:
        pgo_out_pose: torch.tensor, [batch_size*robot_num, (robot_num-1), 7]
    '''
    bs = int(mn_out_pos.size(0) / robot_num)
    ### update pose ###
    update_pose = prior_pose.reshape(-1, robot_num-1, 7) # [batch_size*swarm, (robot_num-1), 7]
    update_pose[:, :, :3] = mn_out_pos
    update_pose = update_pose.reshape(-1, robot_num, robot_num-1, 7) # [batch_size, robot_num, (robot_num-1), 7]

    ### padding ###
    poses_all = torch.zeros(bs, robot_num, robot_num, 7).to(device) # [batch_size, robot_num, robot_num, 7]
    ...
    for ref_id in range(robot_num):
        param_pose_xyzwxyz = poses_all_xyzwxyz[:, ref_id, :, :] # [batch_size, robot_num, 7]
        #### build and run pgo ####
    ...
    pgo_out_pose = torch.zeros(bs, robot_num, robot_num-1, 7).to(device) 
    for i in range(robot_num):
        cnt = 0
        for j in range(robot_num):
            if i == j:
